...
- **CKEditor 5** از CDN نسخهٔ 44.1.0 با `licenseKey: 'GPL'` بارگذاری می‌شود.
  آپلود تصویرِ داخل ادیتور با `Base64UploadAdapter` انجام می‌شود؛ بنابراین
  **نیازی به هیچ Handler/ASHX سمت سرور نیست** و خروجی فقط همان ۳ فایل می‌ماند.
- شمارش کلمات و زمان مطالعه با پلاگین `WordCount` به‌صورت زنده محاسبه می‌شود.
